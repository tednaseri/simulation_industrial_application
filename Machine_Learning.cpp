...
1) Precipitate Class
2) Equations Class
In the following, there are three functions from Equations class.
These functions are for implementing KNN machine learning algorithm.
*/

void equations::prepareKNN() {
	/*
	This function will prepare data for KNN machine learning model.
	At first, it reads the datasets which are already built and saved in data folder.
	Then it will pass the data into KNN model.
	This model will be called later by other function.
	Although this function does not return any variable, the KNN model is saved as a variable in Equations class.
	*/
...
void equations::findNeighbor() {
	/*
	After reading data and preparing KNN model, this function will be ready to find the nearest neighbors of a given point.
	To run the machine learning model, there are three input parameters which can be accessed through:
	temknn, cbLst[0], and cbLst[1]
	We do not pass these variables into this function, because these are variables from equations class.
	It is possible to change the value of K-Nearest number. Here we used k=2.
	After finding the index of nearest neighbors, the target variables will be calculated using neighbors' index.
	Although this function does not return any variable, the measured values of target variables are saved 
	inside "resLst array" as a variable from Equations class.
	The results are also written in a text file.
	*/
...
void equations::setTempValues(int flag, int rowId, double& temp1, double& temp2, double& temp3, double& temp4, double& temp5, double& temp6, double& temp7, double& temp8) {
	/*
	There are 5 different datasets. We have to use the correct dataset according to the given temperature. 
	This function will help the progrom to use the appropriate dataset based on the temperature variable.
	So, target variables can be calculated correctly.
	*/
	if (flag == 1) {
